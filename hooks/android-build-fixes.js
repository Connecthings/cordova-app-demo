...
module.exports = function(context) {
    ...
    // Then, we update the build.gradle version to 3.2.0
    ...
    // Finally, we change the wrapper version to at least the 4.6
...
function updateBuildVersion(gradlePath) {
    if (fs.existsSync(gradlePath)) {
        ...
        var newData = data.replace(gradleRegex, "com\.android\.tools\.build:gradle:3.2.0");
...
function changeWrapperVersion(wrapperPath) {
    if (fs.existsSync(wrapperPath)) {
        ...
        var newData = data.replace(wrapperRegex, "gradle-4.6-all.zip");
